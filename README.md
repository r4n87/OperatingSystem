...
03. 메모리 관리자가 수행하는 세 가지 작업에 대해 설명하시오.  
: summary
...
02. 페이징 기법의 주소 변환 과정을 그림으로 그리고 설명하시오.
...
: 작업집합 모델 workspace model

17. 동적 프레임 할당 방식 중, 페이지 부재 비율의 상하넌과 하한선을 설정하고 페이지 부재 비율이 상한선을 초과하면 할당 프레임을 늘려주는 것은 무엇인가?  
: 페이지 부재 빈도 page fault frequency


### 심화문제
01. 요구 페이징의 의미와 효과를 설명하시오.
- 페이지를 요구할 때 메모리에 올리는 것
- 메모리를 효율적으로 관리
- 응답 속도 향상

02. 세그먼테이션 오류와 페이지 부재의 차이를 설명하시오.
- 세그먼테이션 오류는 프로세스가 주어진 메모리 공간을 벗어나거나, 접근 영역을 벗어났을 때 발생
- 페이지 부재는 페이지를 요청했을 때, 페이지가 메모리에 올라오지 않은 상황
...
- 페이지 교체 시기에, 가장 최근에 사용되지 않은 페이지를 스왑 아웃 swap out 함
- 페이지 접근 시간, 카운터, 참조 비트 형식 구현
- 접근 시간이나 참조 비트 메모리 공간이 별도로 필요함

06. NUR 페이지 교체 알고리즘의 동작을 설명하시오.
- 최근에 미사용한 페이지를 교체하는 알고리즘
- 참조 비트와 변경 비트를 참조하여 victim page를 선정함
- 변경 비트를 먼저 참조, 참조 비트를 나중에 참조

06. 프레임 할당 방식 중 정적 할당에 대해 설명하시오.
- 전체 프로세스에 동일한 크기의 프레임을 할당함 (균등 할당)
- 프로세스 크기에 비례하여 프레임 할당 (비례 할당)
- 유동적 반영이 어려움
- 공간 낭비

07. 프레임 할당 방식 중 동적 할당에 대해 설명하시오.
- 시시각각 변하는 요청에 따라 프레임 할당
- 작업집합 모델, 페이지 부재 빈도 방식


## Chapter 10. I/O System and Storage
### 정리
컴퓨터
- CPU
- 메모리
...
주변장치
- 저속 주변장치: 데이터 전송률이 낮은 장치, 키보드
- 고속 주변장치: 데이터 전송률이 높은 장치, 하드디스크
- 메인보드 내의 버스로 연결
- 데이터가 지나다니는 통로는 채널

입출력 버스
- 이전: CPU가 직접 입출력 장치에서 데이터를 가져옴, 폴링 polling
- 지금: 입출력 제어기 I/O controller가 입출력을 관리함
- 입출력 제어기는 메인버스(CPU, 메모리) + 입출력 버스(주변장치)
- 고속 입출력 버스/저속 입출력 버스로 분리하여 관리
- 채널 선택기: 두 버스 사이의 데이터 전송 관리
- 그래픽카드는 메인버스에 바로 연결
- 메인버스(CPU-메모리), 그래픽 버스(CPU-그래픽카드), 고속 입출력 버스, 저속 입출력 버스
- 버스 채널: 주소 버스, 데이터 버스, 제어 버스

직접 메모리 접근
- DMA Direct Memory Access
- CPU의 도움 없이 메모리에 접근할 수 있도록 입출력 제어기에 부여된 권한
- 입출력 제어기에는 DMA 제어기가 있음
- 주변장치 - DMA 제어기 - 메모리
- 메모리 - DMA 제어기 - 채널 선택기
- 메모리 맵 입출력 memory mapped I/O: 메인 메모리의 주소 공간 중 일부를 DMA 제어기에 할당

인터럽트
- IRQ: 고유 인터럽트 번호
- 외부 인터럽트: 입출력 장치로부터 오는 인터럽트뿐 아니라 기계적 오류 인터럽트 포함, 하드웨어 인터럽트
- 내부 인터럽트: 프로세스 오류와 관련된 인터럽트, 예외 상황 인터럽트
- 자발적 인터럽트: 사용자의 의지로 발생, 시그널
- 인터럽트 벡터: 인터럽트 중 어떤 인터럽트가 발생했는지 파악하기 위해 사용하는 자료 구조
- 인터럽트 핸들러: 인터럽트의 처리 방법을 함수 형태로 만들어 놓은 것

버퍼링
- 버퍼: 속도가 다른 두 장치의 속도 차이 완화
- 단일 버퍼 < 이중 버퍼
- 버퍼가 다 차지 않으면 입출력 장치에 자료가 전송되지 않는 문제 발성
- 플러시 flush: 하드웨어 안전 제거 사용 시 버퍼가 다 안차도 강제로 버퍼 내용을 저장장치로 이용

하드디스크
- 메모리 보조장치, 제 2저장장치
- 성능을 높이려면 탐색 시간 최소화 필요
- 디스크 스케줄링 기법 사용

파티션
- 저장장치 구매 후 처음 사용 시 진행하는 작업
- 디스크를 논리적으로 분할하는 작업
- 마운트mount: 파티션을 하나로 통합하는 기능

포매팅
- 디스크 표면을 초기화하여 사용할 수 있는 형태로 만드는 작업
- 빈 저장장치에 파일 테이블을 탑재하는 작업
- 빠른 포매팅: 데이터는 그대로 둔 채 파일 테이블 초기화
- 느린 포매팅: 일반적인 포매팅, 전체 초기화
- 로 레벨 포매팅: 파일 시스템 뿐만 아니라 파티션, 섹터, 트랙 정보까지 초기화 (공장초기화)

조각 모음
- 조각화, 단편화 정리
- 반도체 저장장치는 조각 모음을 하지 않아도 됨

네트워크 저장장치
DAS
- Direct Attached Storage
- 서버와 같은 컴퓨터에 직접적으로 연결된 저장장치
- 여러 컴퓨터 중 하나를 파일 공유 서버로 지정
- 메인 보드에 있는 입출력 버스와 연결
- SATA Serial Advanced Technology Attachment 방식이 널리 사용됨
- (-) 다른 운영체제가 쓰는 파일 시스템을 쓸 수 없음
- (-) 데이터의 관리나 백업을 사용자가 직접 해야 함

NAS
- Network Attached Storage
- 기존의 저장장치를 LAN이나 WAN에 붙여서 사용
- NAS 전용 운영체제를 가진 독립적인 장치
- 저장장치를 네트워크상에 두고 여러 클라이언트가 네트워크를 통해 접근하게 함
- (+) 공유 데이터의 관리 및 데이터 중복 회피가 가능

SAN
- Storage Area Network
- 데이터 서버, 백업 서버, RAID 등의 장치를 네트워크로 묶고 데이터 접근을 위한 서버를 두는 형태
- (+) 사용자가 파일 관리에 신경을 쓸 필요 없음

디스크 스케줄링
- Disk Scheduling
- 트랙의 이동을 최소화하여 탐색 시간을 줄임
...
블록 SSTF 디스크 스케줄링
- 큐에 있는 트랙 요청을 블록 형태로 묶음
- 모든 트랙이 블록 안에서만 움직임
- (+) 에이징을 사용하여 공평성 보장
- (-) 성능은 FCFS보다 좋지 않음

SCAN 디스크 스케줄링
- SSTF 공평성 위배 문제를 완화하기 위한 기법
- 헤드가 한 방향으로만 움직이면서 서비스
- 엘리베이터 기법
- (+) SSTF보다 공평성을 덜 위배, 성능이 좋음
- (-) 바깥쪽 트랙이 아사 현상

C-SCAN 디스크 스케줄링
- Circular SCAN
- 헤드가 한방향으로 움직이고 서비스가 끝나고, 돌아올 때는 서비스하지 않고 이동만 함
- (-) 작업없이 헤드를 이동하는 비효율성
- (-) 바깥쪽 트랙이 아사 현상

LOOK 디스크 스케줄링
- SCAN 디스크 스케줄링의 불필요한 부분을 제거
- 더 이상 서비스할 트랙이 없으면 헤드가 끝까지 가지 않고 중간에서 방향을 바꿈
- (+) SCAN보다 성능이 좋아 많이 사용됨

C-LOOK 디스크 스케줄링
- Circular LOOK
- 더 이상 서비스할 트랙이 없으면 헤드가 중간에서 방향을 바꿀 수 있음

SLTF 디스크 스케줄링
- Shortest Latency Time First
- 헤드 지지대를 고정하고, 여러 개의 헤드를 지지대에 부착
- 탐색 시간이 없어서 매우 빠르게 데이터를 주고받을수 있음
- 섹터의 순서를 디스크가 회전하는 방향에 맞추어 다시 정렬
- (-) 매우 고가

RAID
- Redundant Array of Independent Disks
- 자동으로 백업, 장애가 발생하면 이를 복구
- 미러링
...
- (-) 장애 발생 시 복구하는 기능이 없기 때문에 장애가 발생하면 데이터를 잃음

RAID1
- 미러링 mirroring
- 하나의 데이터를 2개의 디스크에 나누어 저장, 장애 시 백업 디스크를 활용
- (+) 비용 증가
- (-) 똑같은 내용을 두 번 저장하기 때문에 속도가 느림

RAID2
- 오류 검층 없는 디스크에 대해 오류 교정 코드(ECC) 따로 관리
- 오류가 발생하면 ECC를 통해 디스크 복구
- (+) RAID1보다 작은 저장 공간 요구
- (-) 오류 교정 코드를 계산하는데 많은 시간을 소비하여 사용되지 않음

RAID3
- 오류 검출 코드인 패리티 비트를 사용해 데이터 복구
- 패리티 비트를 통해 오류 검출 및 복구 수행
- N-way 패리티 비트 방식
- (+) 추가되는 디스크 양이 적음
- (-) N-way 패리티 비트를 구성하는 데 필요한 계산량이 많음

RAID4
- RAID3과 같은 방식이지만, 처리하는 데이터가 블록 단위
- RAID3: 모든 디스크가 동시에 동작해야 하는 단점
- (+) 데이터가 저장되는 디스크와 패리티 비트가 저장되는 디스크만 동작
- (+) 추가되는 디스크의 양이 적음
- (-) 패리티 비트 추가 계산량이 많음

RAID5
- RAID4의 병목 현상 해결
- RAID4의 패리티 비트를 여러 디스크에 분산하여 구성
- 패리티 비트를 해당 데이터가 없는 디스크에 보곤
- (+) 한 디스크에 장애가 발생하면 다른 디스크에 있는 패리티 비트를 이용하여 데이터 복구

RAID6
- RAID5와 같은 방식이지만 패리티 비트가 2개
- RAID5: 디스크 2개에 동시에 장애가 발생하면 패리티 비트가 사라짐
- (+) 디스크 2개의 장애 복구 가능
- (-) RAID5보다 계산량이 많고, 추가 디스크 양이 많음
...
- RAID 1,5,6 장단점과 동일함
- 추가되는 디스크의 수가 적지만 입출력 계산량이 증가

### 연습문제
01. 전원 이상이나 기계적인 오류 때문에 발생하는 인터럽트는 무엇인가?  
: 외부 인터럽트

02. 숫자를 0으로 나누거나 자신의 주소 공간을 벗어나서 작업을 하는 것과 같이 프로세스의 오류와 관련된 인터럽트는 무엇인가?  
: 내부 인터럽트

03. 자발적 인터럽트로서 사용자의 의지로 발생시키는 인터럽트는 무엇인가?  
: 시그널

04. 인터럽트의 처리 방법을 함수로 만들어놓은 것을 무엇이라 하는가?  
: 인터럽트 핸들러

05. 하드디스크에서 사용하는 방식으로, 디스크가 일정 속도로 회전하며 모든 트랙의 섹터 개수가 같고 섹터의 크기가 각각 다른 방식은 무엇인가?  
: 각속도 일정 방식의 회전

06. CD에서 사용하는 방식으로, 어느 트랙이나 시간당 디스크 이동 거리가 같고 섹터의 크기가 일정한 방식은 무엇인가?  
: 선속도 일정 방식의 회전

07. 하드디스크 장치의 데이터 전송 시간 중, 현재 위치에서 원하는 트랙까지 이동하는 데 걸리는 시간을 무엇이라 하는가?  
: 탐색 시간 seek time

08. 하드디스크 장치의 데이터 전송 시간 중, 디스크가 회전하여 원하는 섹터를 만날 때까지 걸리는 시간을 무엇이라 하는가?  
: 회전 지연 시간 rotational latency time

09. 네트워크 저장장치 중 저장장치를 네트워크상에 두고 여러 클라이언트가 네트워크를 통해 접근함으로써 공유 데이터의 관리 및 데이터의 중복 회피가 가능한 기법은 무엇인가?  
: NAS
...
12. 헤드가 움직이기 시작하면 맨 마지막 트랙에 도착할 때까지 뒤돌아가지 않고 계속 앞으로 전진하는 디스크 스케줄링 방법은 무엇인가?  
: SCAN
...
### 심화문제
01. 입출력 버스의 구조를 그림으로 그리고 설명하시오.
- 메인 버스 - CPU, 그래픽 카드 연결
- 고속 입출력 버스, 저속 입출력 버스 (채널 선택기를 통해 소통)
- 메인 버스와는 입출력 제어기를 통해 연결

02. 인터럽트의 종류를 나열하고 설명하시오.
- 외부 인터럽트: 입출력 장치로부터 오는 인터럽트뿐 아니라 기계적 오류 인터럽트 포함, 하드웨어 인터럽트
- 내부 인터럽트: 프로세스 오류와 관련된 인터럽트, 예외 상황 인터럽트
- 자발적 인터럽트: 사용자의 의지로 발생, 시그널

03. 각속도 일정 방식과 선속도 일정방식에 대해 설명하시오.  
각속도 일정 방식
- 하드 디스크에서 사용
- 항상 일정한 속도로 회전, 바깥쪽 트랙의 속도가 안쪽 트랙 속도보다 빠름
- 바깥쪽 섹터가 안쪽 섹터보다 더 큼

선속도 일정 방식
- CD에서 사용
- 어느 트랙에서나 단위 시간당 디스크의 이동 거리가 같음
- 안쪽 트랙에 있을 때는 회전 속도를 빠르게, 바깥쪽 트랙에 있을 때는 회전 속도를 느리게 해야 함

04. SSTF 디스크 스케줄링의 장단점을 설명하시오.
- (+) 효율성이 좋음
- (-) 아사 현상 발생

05. LOOK 디스크 스케줄링이 만들어진 이유를 설명하시오.
- SCAN 디스크 스케줄링의 비효율성 극복
- SCAN은 트랙 요청이 없어도 맨 마지막 트랙에 도착한 후에야 방향을 바꿈
- LOOK은 더 이상 서비스할 트래기 없으면 헤드가 끝까지 가지 않고 중간에서 방향을 바꿈

06. C-SCAN 디스크 스케줄링에 대해 설명하시오.
- SCAN 디스크 스케줄링 변형
- 헤드가 한쪽 방향으로 움직일 때는 요청받은 트랙 서비스, 돌아올 때는 서비스 하지 않고 이동만
- (-) 서비스 없이 헤드를 이동하는 비효율성
- (-) 바깥쪽 트랙 아사 현상

07. RAID 10과 RAID 0+1을 비교하여 설명하시오.
- RAID 0+1: 장애가 발생했을 때, 복구하기 위해 모든 디스크를 중단해야 함
- RAID 10: 장애가 발생했을 때, 일부 디스크만 중단하여 복구 가능 (2개의 디스크가 묶여 있기 때문)
...
- size, time: 크기와 접근 시간
- location: 디렉터리, 위치
- accessibility, owner: 접근권한, 소유자
- 파일 헤더: 파일 테이블에서 관리, 이름 종류 크기 시간 접근 권한과 가은 일반적인 내용
- 고유 헤더: 데이터 파일에 파일의 버전 번호, 크기, 특수 정보 등 응용 프로그램이 필요로 하는 정보

파일 작업
- 파일 작업 or 파일 연산 file operation
- 파일 자체를 변경하는 작업: open, close, create, remove, copy, rename, list, search
- 파일 내용을 변경하는 작업: open(), create(), close(), read(), write(), update(), insert(), delete()

파일 구조
순차 파일 구조 
- sequential file structure
- lseek() 명령을 통해 순차적으로 접근 가능
- 순서대로 기록되기 때문에 낭비되는 부분이 없음
- 구조가 단순하여 타 저장장치에도 적용 가능
- 순서대로 데이터를 읽거나 저장 시 빠르게 처리 됨
- (-) 새로운 데이터 삽입 및 삭제 시 시간이 많이 걸림
- (-) 특정 데이터로 이동 시 접근이 어려움

인덱스 파일 구조
- index file structure
- 순차 파일 구조의 단점 해결
- 순차 파일 구조에 인덱스 테이블을 추가하여 순차 접근, 직접 접근 가능
- indexed sequential access / ISAM Index Sequential Access Method
- 순차 접근, 직접 접근 모두 가능
- 저장 시에는 순차 파일 구조로, 접근 시에는 인덱스 테이블 사용하여 직접 접근

직접 파일 구조
- direct file structure
- 저장하려는 데이터의 특정 값에 어떤 관계를 정의하여 물리적 주소로 바로 변환하는 파일 구조
- 해시 함수 사용 hash function
- (+) 데이터 접근이 매우 빠름
- (-) 해시 함수 선정이 어려움
...
- 디렉터리 이동 시 cd(change directory) 명령 사용
...
마운트
- 여러 개의 파티션을 통합하는 명령어
- 하나의 파일 테이블에 여러 개의 파티션을 붙여서 사용
- 마운트 연결 지점 mounting point 필요
...
- 실제로 사용되지 않음
- (-) 디스크에 남은 공간 중 파일 크기와 맞는 연속된 공간이 없을 경우에는 연속 할당이 불가능

불연속 할당
- non-contiguous allocation
- 비어있는 블록에 데이터를 분산하여 저장하고 파일 시스템이 정보 관리
- (ex) 연결 할당, 인덱스 할당

연결 할당
- linked allocation
- 파일에 속한 데이터를 연결 리스트로 관리
- 시작 블록에 대한 정보만 저장, 나머지 데이트는 시작 블록부터 연결하여 저장
- = chained allocation
- 테이블 형태로 관리

인덱스 할당
- indexed allocation
- 연결 리스트는 최대 할당 크기에 제한이 있음. 이를 해결하기 위한 방식
- 데이터의 인덱스를 담고 있는 인덱스 블록을 연결함
- 간접 인덱스 블록을 만들어 테이블 무한 확장 가능
- 파일 제어 블록, 블록 포인터, 간접 포인터, 이중 간접 포인터, 삼중 간접 포인터 구성

### 연습문제
01. 파일을 사용할 때 운영체제로부터 부여받은 접근 권한을 무엇이라 하는가?  
: 디스크립터

02. 파일 구조 중 처음부터 순서대로 접근하는 구조는 무엇인가?  
: 순차 파일 구조

ㅇ03. 파일 구조 중 순차 접근과 직접 접근이 모두 가능한 구조는 무엇인가?  
: 인덱스 파일 구조

04. 저장하려는 데이터의 특정 값에 어떤 관계를 정의하여 물리적인 주소로 바로 변환하는 파일 구조는 무엇인가?  
: 직접 파일 구조

05. 직접 파일 구조에서 데이터 변환에 사용하는 함수는 무엇인가?  
: 해시 함수

06. 파일 전체 디렉터리 가운데 어느 위치에 있는지 나타내는 정보는 무엇인가?  
: 경로

07. 루트 디렉터리를 기준으로 정하는 경로와 현재 디렉터리를 기준으로 정하는 경로를 각각 무엇이라 하는가?  
: 절대 경로, 상대 경로

08. 여러 개로 나뉜 파티션을 하나로 묶는 명령어는 무엇인가?  
: 마운트

09. 디스크 할당 방식에서 파일에 속한 데이터를 연결 리스트로 관리하는 방식은 무엇인가?  
: 연결 할당
...
### 심화문제
01. 순차 파일 구조에 대해 설명하시오.
- 파일에 순차적으로 접근
- 순차적으로 검색 시에 빠르게 접근 가능
- 중간 삽입, 삭제가 어려움

02. 인덱스 파일 구조에 대해 설명하시오.
- 인덱스를 통해 파일에 접근 가능
- 순차 파일 구조의 단점을 극복하기 
- 순차 접근, 직접 접근 모두 가능

03. 직접 파일 구조에 대해 설명하시오.  
- 해시 함수를 사용하여 데이터에 대한 물리 주소로 변환
- 데이터에 대한 접근이 빠르나, 해시 함수 선정에 대한 어려움이 있음
